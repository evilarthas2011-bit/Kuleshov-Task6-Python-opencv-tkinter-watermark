...
class VisionProcessor:
    ...
    def apply_watermark_effect(self, text, blur_strength):
        ...
        # 1. Сильно размываем всю матрицу
        result = cv2.blur(self._image, (blur_strength, blur_strength))
        
        # 2. Вычисляем размеры центральной зоны (30% от размера изображения)
        h, w = self._image.shape[:2]
        ...
        # 3. Возвращаем оригинальные резкие пиксели в центральный прямоугольник
        result[y1:y2, x1:x2] = self._image[y1:y2, x1:x2]

        # 4. Наносим водяной знак методом cv2.putText
        font = cv2.FONT_HERSHEY_SIMPLEX
        # Динамический расчет масштаба шрифта под размер картинки
        font_scale = w / 1000.0 if w > 500 else 0.7
        thickness = max(1, int(font_scale * 2))
        
        # Получаем размер текста для идеального центрирования внутри рамки
        text_size = cv2.getTextSize(text, font, font_scale, thickness)[0]
        text_x = cx - text_size[0] // 2
        text_y = cy + text_size[1] // 2

        # Наносим текст (белый цвет, BGR формат)
...
class App(tk.Tk):
    def __init__(self):
        ...
        # Создание интерфейса с вкладками (Notebook)
        ...
        # Выпадающий список размытия (Combobox)
        ...
        # Контейнеры для отображения изображений
    ...
    def process_image(self):
        ...
        # Показ на вкладке "Обработка" и автоматическое переключение
    ...
    def display_image(self, cv_img, target_label):
        ...
        # Меняем каналы BGR -> RGB
        rgb_img = cv2.cvtColor(cv_img, cv2.COLOR_BGR2RGB)
        img_pil = Image.fromarray(rgb_img)
        
        # Пропорциональное масштабирование под размеры окна (макс 600х450)
        img_pil.thumbnail((600, 450))
        
        img_tk = ImageTk.PhotoImage(image=img_pil)
        target_label.configure(image=img_tk)
        target_label.image = img_tk  # Сохраняем ссылку, чтобы картинка не исчезла
